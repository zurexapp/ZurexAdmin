import React, { useState } from "react";
import { getStatusColorNew } from "../utils/helpers";

const ShowTeamInfo = ({ teamInfo }) => {
  const [shoMembers, setshoMembers] = useState(false);
  return (
    <div className="flex gap-6">
      {teamInfo?.members?.length > 0 ? (
        <div className=" flex items-start justify-center flex-col gap-5 shrink-0">
          <button
            className="badge-status badge-status--lg"
            style={{
              backgroundColor: `var(--${getStatusColorNew(
                !shoMembers ? "active" : "inactive"
              )})`,
            }}
            onClick={() => setshoMembers(!shoMembers)}
          >
            Show {shoMembers ? "Less" : "More"}
          </button>

          {shoMembers ? (
            <>
              <h5 className="text-sm">City:{teamInfo?.cityName}</h5>
              <h5 className="text-sm my-1">Members:</h5>
              {teamInfo?.members?.map((dat) => (
                <div className="flex items-center justify-start flex-row mb-1">
                  <div className="img-wrapper w-[50px] h-[50px] flex items-center justify-center shrink-0 mr-[10px]">
                    <img
                      style={{
                        objectFit: "cover",
                        width: "100%",
                        height: "100%",
                      }}
                      src={
                        dat?.userImage
                          ? dat?.userImage
                          : "data:image/png;base64,"
                      }
                      alt={"profilePicture"}
                    />
                  </div>
                  <div className="flex items-start justify-center flex-col">
                    <h6 className="text-xs">{dat.name}</h6>
                    <h6 className="text-xs">{dat.phone}</h6>
                  </div>
                </div>
              ))}
            </>
          ) : null}
        </div>
      ) : (
        <div className=" flex items-start justify-center flex-col shrink-0">
          <h5 className="text-sm">N/A</h5>
        </div>
      )}
    </div>
  );
};

export default ShowTeamInfo;
